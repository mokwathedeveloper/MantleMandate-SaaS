...
| 🗺️ [Final_Architecture_for_MantleMandate_SaaS.md](Final_Architecture_for_MantleMandate_SaaS.md) | System architecture diagram, 4 layers, data flow (mandate → agent → trade), security table, CI/CD pipeline |
...
| 🗓️ [Full_Stack_Implementation_Roadmap.md](Full_Stack_Implementation_Roadmap.md) | Phased build plan (Phase 1–6): setup commands, what to build in what order, submission checklist |
| 🗂️ [Full_Stack_Implementation_Roadmap(2).md](Full_Stack_Implementation_Roadmap(2).md) | Complete folder/file structure — every `.py`, `.tsx`, `.ts`, `.sol` file listed with its purpose |
...
|----------|-----------------|
| 🖼️ [../uiux-v2/](../uiux-v2/) | **All 20 screen specs** — exact colors, fonts, component layouts, states, empty states |
...
| ♟️ [Turing_Test_Hackathon_Strategy.md](Turing_Test_Hackathon_Strategy.md) | Winning strategy: Phase 1 (ClawHack), Phase 2 (AI Awakening), judging criteria breakdown |
...
> **Note:** This table was written for the project's *original architecture plan*, which was implemented as the reference backend in [`backend/`](../backend/). The **live demo** consolidated the backend-related decisions onto Next.js API routes + Supabase instead — see the **Live Demo** column below, and [Backend Implementations: Live vs. Reference](../README.md#-backend-implementations-live-vs-reference) in the root README for the full picture. Frontend, contracts, and product decisions are unchanged and apply to both.

| Decision | Original Plan / Reference (`backend/`) | Live Demo |
|---------|--------|--------|
| Backend language | Python (Flask 3.x) — NOT Node.js | Next.js 14 API Routes (TypeScript), `frontend/app/api/*` |
| Backend pattern | Application Factory + Blueprints | App Router Route Handlers |
| Frontend framework | Next.js 14 TypeScript, App Router (`app/`) | *(same)* |
| File extensions | `.tsx` components, `.ts` utils/hooks | *(same)* |
| Database | PostgreSQL via SQLAlchemy ORM + Flask-Migrate | Supabase Postgres |
| Auth | Flask-JWT-Extended (Python) | Supabase Auth |
| Passwords | flask-bcrypt (Python) | Supabase Auth (managed) |
| Background jobs | Celery + Redis | None — agent ticks are run on-demand from the dashboard ("Run Trading Cycle") |
| Real-time | Flask-SocketIO ↔ socket.io-client | Not implemented (`socket.io-client` is an unused dependency) |
| Blockchain (backend) | web3.py | viem server wallet (`frontend/lib/serverWallet.ts`) |
| Blockchain (frontend) | Ethers.js v6 + wagmi | wagmi + viem (no ethers.js) |
| Smart contracts | Hardhat + Solidity 0.8.x (NOT Truffle) | *(same)* |
| Backend testing | pytest + pytest-flask (NOT Mocha/Chai) | *(same, for `backend/` only)* |
| Frontend testing | Jest + React Testing Library | *(same)* |
| Global state | Zustand (NOT Redux) | *(same)* |
| Server state | TanStack Query | *(same)* |
| Deployment (frontend) | Vercel | Vercel — [live](https://mantle-mandate-saa-s.vercel.app) |
| Deployment (backend) | Railway or Render | None deployed — not part of the live demo |
| Product name | MantleMandate (no hyphen, no "-SaaS") | *(same)* |
| Plans | Operator ($29) · Strategist ($99) · Institution ($299) | *(same)* |
| Crypto payments | USDC · USDT · MNT (10% MNT discount) | *(same)* |
