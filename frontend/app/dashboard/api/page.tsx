...
function AddIntegrationModal({ onClose, t }: { onClose: () => void; t: (s: string) => string }) {
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3.5 bg-surface border-b border-border">
          <div className="flex items-center gap-2.5">
            ...
            <span id="add-integration-title" className="text-[13px] font-semibold text-text-primary">{t('Add New Integration')}</span>
          </div>
          <button onClick={onClose} className="text-text-secondary hover:text-text-primary transition-colors" aria-label={t('Close modal')}>
            ✕
          </button>
        </div>
        ...
        <div className="p-5">
          {step === 'type' && (
            <div className="space-y-3">
              <p className="text-xs text-text-secondary">{t('Select the type of data source you want to add:')}</p>
              <div className="grid grid-cols-2 gap-2">
                {INTEGRATION_TYPES.map(it => (
                  <button
                    key={it.id}
                    onClick={() => setKind(it.id)}
                    className={cn(
                      'text-left p-3 rounded-lg border transition-all',
                      kind === it.id ? 'bg-primary/10 border-primary' : 'bg-page border-border hover:border-text-disabled'
                    )}
                  >
                    <p className="text-xs font-semibold text-text-primary">{t(it.label)}</p>
                    <p className="text-[11px] mt-0.5 text-text-secondary">{t(it.description)}</p>
                  </button>
                ))}
              </div>
          ...
          {step === 'config' && (
            <div className="space-y-3">
              <p className="text-xs text-text-secondary">
                {t('Configure your')} <span className="text-text-primary font-semibold">{t(INTEGRATION_TYPES.find(it => it.id === kind)?.label ?? '')}</span> {t('integration:')}
              </p>
              ...
              <div className="flex gap-2 justify-end pt-1">
                ...
                >
                  ...
                  {saving ? t('Saving…') : t('Save Integration')}
                </button>
              </div>
            </div>
          )}
          ...
          {step === 'done' && (
            <div className="flex flex-col items-center gap-3 py-6 text-center">
              ...
              <div>
                <p className="text-sm font-semibold text-text-primary">{name} {t('added')}</p>
                <p className="text-xs mt-1 text-text-secondary">{t('Integration saved. Data will begin flowing in shortly.')}</p>
              </div>
              <button
                onClick={onClose}
    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div>
          <h2 className="text-2xl font-bold text-text-primary">
            {t('API Integration & Data Ingestion')}
          </h2>
          <p className="text-sm mt-0.5 text-text-secondary">
            {t('Live market data connections, on-chain state, and ingestion logs.')}
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 self-start">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-success-bg border border-success/40">
            ...
            <span className="text-xs font-semibold text-success">{t('SYSTEM OPERATIONAL')}</span>
            <span className="text-xs ml-1 text-text-secondary">{t('All APIs Connected')}</span>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {([
          ...
          <div key={c.label} className="bg-card border border-border rounded-lg p-4">
            ...
            <p className="text-sm font-bold text-text-primary">{c.label === 'DeFi Protocols' ? t(c.value) : c.value}</p>
            <span className="text-[10px] font-semibold text-success">{t(c.status)}</span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-10 gap-4 items-start">
        ...
        <div className="md:col-span-1 lg:col-span-3">
          <div className="bg-card border border-border rounded-lg p-5 space-y-4">
            <h4 className="text-sm font-semibold text-text-primary">{t('Integration Settings')}</h4>

            {/* Bybit Market Data */}
            <div className="space-y-2.5">
              <div className="flex items-center gap-1.5">
                <span className="h-2 w-2 rounded-full bg-success" />
                <span className="text-xs font-semibold text-success">{t('CONNECTED')}</span>
                <span className="text-xs ml-0.5 text-text-secondary">Bybit Market Data</span>
              </div>

              <div className="flex items-center justify-between gap-2">
                <span className="text-[11px] shrink-0 w-20 text-text-disabled">{t('API Key:')}</span>
              ...
              {([
                ...
                <div key={k} className="flex gap-2 text-[11px]">
                  <span className="shrink-0 w-20 text-text-disabled">{t(k)}:</span>
                  <span className="font-mono text-text-secondary">{k === 'Permissions' ? t(v) : v}</span>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-border pt-4 space-y-2.5">
              ...
              <div className="flex items-center justify-between gap-2">
                <span className="text-[11px] shrink-0 w-20 text-text-disabled">{t('RPC Endpoint:')}</span>
                <div className="flex items-center gap-2 flex-1 justify-between">
              ...
              {([
                ...
                <div key={k} className="flex gap-2 text-[11px]">
                  <span className="shrink-0 w-20 text-text-disabled">{t(k)}:</span>
                  <span className="font-mono text-text-secondary">{k === 'Block time' ? t(v) : v}</span>
                </div>
              ))}
            </div>
        ...
        <div className="md:col-span-1 lg:col-span-4">
          <div className="bg-card border border-border rounded-lg p-5 space-y-4">
            <h4 className="text-sm font-semibold text-text-primary">{t('Response Preview')}</h4>
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-card border border-border rounded-lg p-5 space-y-3">
            <h4 className="text-sm font-semibold text-text-primary">{t('Recent Inbound')}</h4>
            ...
            {TICKERS.map(tk => (
              ...
              >
                <span className="w-20 shrink-0 text-text-secondary">{tk.sym}</span>
                <span className="text-text-primary">{tk.price}</span>
                <div className="flex items-center gap-0.5">
                  {tk.up
                    ? <TrendingUp  className="h-3 w-3 text-success" />
                    : <TrendingDown className="h-3 w-3 text-error" />
                  }
                  <span className={tk.up ? 'text-success' : 'text-error'}>{tk.chg}</span>
                </div>
                <span className="text-text-disabled">{tk.age}</span>
              </div>
            ))}
          </div>
          ...
          <div className="bg-card border border-border rounded-lg p-5 space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-semibold text-text-primary">{t('Error Log')}</h4>
              <button
                onClick={() => setErrors([])}
            ...
            {errors.length === 0 ? (
              <div className="flex items-center justify-center gap-1.5 py-3">
                ...
                <p className="text-xs text-success">{t('No errors in the last 24 hours')}</p>
              </div>
            ) : (
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-border">
          <h4 className="text-sm font-semibold text-text-primary">{t('Ingestion Log')}</h4>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {t('Auto-scroll')} {autoScroll ? t('ON') : t('OFF')}
            </button>
            ...
            <div className="relative" ref={filterRef}>
              ...
              >
                ...
                {filterLevel === 'ALL' ? t('Filter') : filterLevel}
        ...
        <div ref={logsRef} className="bg-page max-h-56 overflow-y-auto overflow-x-auto">
          {filteredLogs.length === 0 ? (
            <p className="text-center py-8 text-xs font-mono text-text-disabled">
              {filterLevel !== 'ALL' ? t('No log entries at level {level}').replace('{level}', filterLevel) : t('No log entries')}
            </p>
          ) : (
            filteredLogs.map(l => (
              <div
        ...
        <div className="flex items-center justify-between px-5 py-2 text-[10px] border-t border-border font-mono text-text-disabled">
          <span>{t('{n} / 200 entries shown').replace('{n}', String(filteredLogs.length))}</span>
          <span className="flex items-center gap-2">
            {errorCount > 0 && (
              <span className="text-error">
                {t(errorCount > 1 ? '{n} errors' : '{n} error').replace('{n}', String(errorCount))}
              </span>
            )}
            {warnCount > 0 && (
              <span className="text-warning">
                {t(warnCount > 1 ? '{n} warnings' : '{n} warning').replace('{n}', String(warnCount))}
              </span>
            )}
          </span>
        </div>
      ...
      {/* ── Add Integration Modal ───────────────────────────────────────────── */}
      {showAddModal && <AddIntegrationModal onClose={() => setShowAddModal(false)} t={t} />}
