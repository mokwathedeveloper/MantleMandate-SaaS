...
function getFaqItems(t: (s: string) => string): FaqItem[] {
  return [
    {
      question: t('How do I deploy my first AI agent?'),
      answer: t('Go to Dashboard → Agents → New Agent. Write your mandate in plain English — for example, "Buy ETH when RSI is below 30, never risk more than 3% per trade." Claude parses it into a structured policy, which is hashed and anchored to Mantle Sepolia via the MandatePolicy contract. Click Deploy and your agent is registered on-chain and starts running automatically.'),
    },
    {
      question: t('What is a policy hash and why does it matter?'),
      answer: t('A policy hash is a keccak256 fingerprint of your mandate\'s rules, stored immutably on the MandatePolicy smart contract on Mantle. It creates a cryptographically verifiable record that your agent\'s decisions are governed by your stated policy. If an agent executes a trade that violates the hash, it is detectable on-chain — giving you a trustless compliance guarantee no centralised system can offer.'),
    },
    {
      question: t('How does the plain-English mandate parser work?'),
      answer: t('The mandate parser uses Claude (claude-sonnet via OpenRouter) to extract structured trading rules from your natural-language input. It identifies: asset pairs, entry/exit triggers (RSI, price levels, time windows), position size limits, and stop-loss conditions. The output is a JSON policy object that is hashed and submitted to MandatePolicy.sol. You can review the parsed JSON before deploying.'),
    },
    {
      question: t('My agent paused unexpectedly — what do I do?'),
      answer: t('Check Dashboard → Protocols → History tab for the last execution log. Common causes: (1) RiskGuard circuit breaker triggered — your drawdown or notional limit was hit; (2) Bybit API key expired or was rate-limited; (3) Mantle Sepolia RPC blip. To resume: resolve the root cause, then go to Agents → select the agent → Resume. If the agent shows "STOPPED" rather than "PAUSED", contact support — a STOPPED agent requires manual review before reactivation.'),
    },
    {
      question: t('How to read the on-chain audit trail?'),
      answer: t('Go to Dashboard → Audit. Each row is an OrderExecuted event emitted by the AgentExecutor contract on Mantle Sepolia. Click the TX hash column to open the transaction on Mantle Explorer. The Mandate Hash column confirms which policy authorised each trade. All events are immutable — they cannot be edited or deleted, providing a tamper-proof compliance record suitable for regulatory review.'),
    },
    {
      question: t('Setting risk limits: drawdown, stop-loss, position sizing'),
      answer: t('In Dashboard → Risk, set: Max Drawdown % (a circuit breaker that pauses the agent when the portfolio drops this amount from its peak), Max Notional per Trade (a hard USD cap per single order), and Stop-Loss % (automatically closes a position when the unrealised loss exceeds this threshold). These limits are enforced by RiskGuard.sol on-chain — agents cannot bypass them regardless of market conditions or mandate instructions.'),
    },
    {
      question: t('How to connect multiple wallets to one account?'),
      answer: t('Currently one primary wallet is linked per account (connected via MetaMask or WalletConnect during onboarding). Multi-wallet support is on the roadmap. You can view your connected wallet in Dashboard → Wallets and manage off-chain funds via the Bybit API integration from Dashboard → Settings → API Keys. Each account maps to one on-chain identity for mandate governance and audit purposes.'),
    },
    {
      question: t('Exporting reports for tax purposes'),
      answer: t('Go to Dashboard → Trades and click Export (top right). You can download all trade history as CSV with columns: Date, Pair, Side, Amount, Price, P&L, Protocol, On-Chain TX. For tax reporting, use the On-Chain TX links to verify each trade independently on Mantle Explorer. The on-chain audit trail at Dashboard → Audit provides an immutable record acceptable for compliance submissions in most jurisdictions.'),
    },
  ]
}

function getResources(t: (s: string) => string): { label: string; href: string; ext: boolean }[] {
  return [
    { label: t('API Documentation'),   href: 'https://docs.mantlemandate.xyz/api',       ext: true  },
    { label: t('Smart Contract Docs'), href: 'https://docs.mantlemandate.xyz/contracts', ext: true  },
    { label: t('Changelog'),           href: 'https://docs.mantlemandate.xyz/changelog', ext: true  },
    { label: t('Status Page'),         href: 'https://status.mantlemandate.xyz',         ext: true  },
    { label: t('Privacy Policy'),      href: '/privacy',                                  ext: false },
    { label: t('Terms of Service'),    href: '/terms',                                    ext: false },
  ]
}

function getPlanLabels(t: (s: string) => string): Record<string, string> {
  return {
    operator:    t('OPERATOR PLAN'),
    strategist:  t('STRATEGIST PLAN'),
    institution: t('INSTITUTION PLAN'),
  }
}
function getPlanSupport(t: (s: string) => string): Record<string, { tier: string; response: string }> {
  return {
    operator:    { tier: t('Standard Support'),   response: t('24-hour response') },
    strategist:  { tier: t('Priority Support'),   response: t('4-hour response')  },
    institution: { tier: t('Enterprise Support'), response: t('24/7 SLA')         },
  }
}
...
function timeAgo(iso: string, t: (s: string) => string): string {
  ...
  if (Number.isNaN(ts)) return t('Unknown time')
  const diff = Math.max(0, Math.floor((Date.now() - ts) / 1000))
  if (diff < 60)    return t('Just now')
  if (diff < 3600)  return t('{n} min ago').replace('{n}', String(Math.floor(diff / 60)))
  if (diff < 86400) return t('{n} hr ago').replace('{n}', String(Math.floor(diff / 3600)))
  return t('{n}d ago').replace('{n}', String(Math.floor(diff / 86400)))
}
...
function PriorityLabel({ priority, t }: { priority: string; t: (s: string) => string }) {
  ...
  return <span className={cn('text-xs font-medium', colors[priority] ?? 'text-text-secondary')}>{PRIORITY_LABEL[priority] ?? priority}</span>
}

function TicketForm({ onClose, onSubmitted, t }: { onClose: () => void; onSubmitted: () => void; t: (s: string) => string }) {
  const user = useAuthStore(s => s.user)
  const plan = user?.plan ?? 'operator'
  const planSupport = getPlanSupport(t)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : t('Failed to submit. Please try again.'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (success) {
    return (
      <div className="border border-success/40 rounded-md p-8 bg-success-bg flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-sm font-semibold text-text-primary">{t('Ticket submitted successfully!')}</p>
        <p className="text-xs text-text-secondary">
          {t("We'll respond within {hours}.").replace('{hours}', planSupport[plan]?.response ?? t('24 hours'))}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="border border-border rounded-md p-4 space-y-3 bg-surface">
      <div className="flex items-center justify-between">
        <p className="text-sm font-semibold text-text-primary">{t('Submit New Ticket')}</p>
      ...
      <div className="space-y-1">
        <label className="text-xs text-text-secondary">{t('Subject')}</label>
        <input
          value={subject}
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          <label className="text-xs text-text-secondary">{t('Priority')}</label>
      ...
      <div className="space-y-1">
        <label className="text-xs text-text-secondary">{t('Message')}</label>
        <textarea
          value={message}
      ...
      <div className="space-y-1">
        <label className="text-xs text-text-secondary">{t('Attachment (optional)')}</label>
...
export default function SupportPage() {
  ...
  const planSupports = getPlanSupport(t)
  const planLabel   = planLabels[plan]  ?? planLabels.operator
  const planSupport = planSupports[plan] ?? planSupports.operator
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setStatusLabel(diff < 1 ? 'Just now' : `{n} min ago|${diff}`)
    }, 60_000)
    return () => clearInterval(interval)
  }, [fetchTickets])

  const displayStatusLabel = statusLabel === 'Just now'
    ? t('Just now')
    : t('{n} min ago').replace('{n}', statusLabel.split('|')[1])

  const query = searchVal.toLowerCase().trim()

  const filteredTickets = tickets.filter(tk =>
    !query ||
    tk.subject.toLowerCase().includes(query) ||
    tk.ticket_ref.toLowerCase().includes(query) ||
    tk.category.toLowerCase().includes(query)
  )
    ...
    <div className="space-y-6">
      ...
      <div>
        <h2 className="text-[32px] font-bold text-text-primary leading-tight">
          {t('Hi {name}, how can we help?').replace('{name}', firstName)}
        </h2>
        <p className="text-sm text-text-secondary mt-1">
          {t('Search our docs, browse FAQs, or reach our team directly.')}
        </p>
        <div className="relative mt-4">
          ...
          <input
            ...
            placeholder={t('Search tickets and help topics…')}
      ...
      <div className="flex items-center gap-3 rounded-[6px] px-4 py-2.5 flex-wrap bg-success-bg border border-success/40">
        ...
        <span className="text-[13px] font-medium flex-1 text-success">{t('All systems operational')}</span>
        <span className="text-[13px] text-text-secondary">{t('Last checked: {time}').replace('{time}', displayStatusLabel)}</span>
        <a
        ...
        >
          {t('Status Page')} <ExternalLink className="h-3 w-3" />
        </a>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        <div className="bg-card border border-border rounded-lg p-5 flex flex-col gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-text-primary">{t('Email Support')}</p>
            <p className="text-xs text-text-secondary mt-1">
              {t('Response within 4 hours for Strategist & Institution plans.')}
            </p>
          </div>
          <EmailWidget />
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-5 flex flex-col gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              <p className="text-sm font-semibold text-text-primary">{t('Live Chat')}</p>
              <span className="text-[10px] font-semibold text-success flex items-center gap-0.5">
                <span className="h-1.5 w-1.5 rounded-full bg-success inline-block" /> {t('Online')}
              </span>
            </div>
            <p className="text-xs text-text-secondary mt-1">
              {t('Available Monday–Friday, 9am–6pm UTC.')}
            </p>
          </div>
          <ChatWidget />
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-5 flex flex-col gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-text-primary">{t('Documentation')}</p>
            <p className="text-xs text-text-secondary mt-1">
              {t('Full guides, API reference, and mandate writing tutorials.')}
            </p>
          </div>
          <a
            href="/dashboard/docs"
          ...
          >
            {t('Browse Docs')} <ExternalLink className="h-3.5 w-3.5" />
          </a>
        </div>
        ...
        <div className="bg-card border border-border rounded-lg p-5 flex flex-col gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-semibold text-text-primary">{t('Community Forum')}</p>
            <p className="text-xs text-text-secondary mt-1">
              {t('Connect with other MantleMandate users. Share strategies.')}
            </p>
          </div>
          <a
            ...
            className="text-sm text-text-link hover:underline underline-offset-2 w-fit flex items-center gap-1"
          >
            {t('Join Forum')} <ExternalLink className="h-3.5 w-3.5" />
          </a>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-10 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-card border border-border rounded-lg p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-semibold text-text-primary">{t('Your Support Tickets')}</h4>
              <button
              ...
              >
                {showForm ? t('Hide Form') : t('Submit New Ticket')}
              </button>
            </div>
            ...
            {ticketsLoading ? (
              <div className="flex items-center justify-center py-10 gap-2 text-text-secondary">
                ...
                <span className="text-sm">{t('Loading tickets…')}</span>
              </div>
            ) : ticketsAuthError ? (
              <div className="flex flex-col items-center gap-2 py-10 text-center">
                <AlertCircle className="h-7 w-7 text-warning" />
                <p className="text-sm text-text-secondary">{t('Your session has expired. Please refresh the page to continue.')}</p>
              </div>
              ...
              <div className="flex flex-col items-center gap-3 py-10 text-center">
                ...
                <p className="text-sm text-text-secondary">
                  {query
                    ? t('No tickets match "{query}"').replace('{query}', searchVal)
                    : t('No tickets yet. Submit your first one above.')}
                </p>
                {!query && (
                  <button
              ...
              <>
                <div className="overflow-hidden border border-border rounded-md">
                  ...
                  >
                    {[t('ID'), t('SUBJECT'), t('STATUS'), t('PRIORITY'), t('UPDATED')].map(h => (
                  ...
                  {shownTickets.map((tk, i) => (
                    ...
                    >
                      <span className="font-mono text-xs text-text-secondary">#{tk.ticket_ref}</span>
                      <span className="text-sm text-left truncate pr-2 text-text-primary">{tk.subject}</span>
                      <StatusBadge status={tk.status} t={t} />
                      <PriorityLabel priority={tk.priority} t={t} />
                      <span className="text-xs text-text-secondary">{timeAgo(tk.created_at, t)}</span>
                    </div>
                  ))}
                </div>
                ...
                {filteredTickets.length > 5 && (
                  <div className="flex justify-end">
                    ...
                    >
                      {showAll
                        ? t('Show fewer')
                        : t('View all {n} tickets').replace('{n}', String(filteredTickets.length))}
          ...
          <div className="bg-card border border-border rounded-lg p-5 space-y-3">
            <h4 className="text-sm font-semibold text-text-primary">{t('Popular Topics')}</h4>

            {filteredFaq.length === 0 ? (
              <p className="text-sm text-text-secondary py-4 text-center">
                {t('No topics match “{query}”').replace('{query}', searchVal)}
              </p>
            ) : (
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          <div className="bg-card border border-border rounded-lg p-5 space-y-1">
            <h4 className="text-sm font-semibold text-text-primary mb-3">{t('Resources')}</h4>
            {getResources(t).map(({ label, href, ext }) => (
              <a
                key={label}
                href={href}
          ...
          {/* Contact Hours */}
          <div className="bg-card border border-border rounded-lg p-5 space-y-4">
            <h4 className="text-sm font-semibold text-text-primary">{t('Contact Hours')}</h4>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold text-text-primary">📧 {t('Email Support')}</p>
              <p className="text-xs text-text-secondary">{t('Mon–Fri, 9am–6pm UTC')}</p>
              <p className="text-xs text-text-secondary">{t('4-hour response (Strategist / Institution)')}</p>
              <p className="text-xs text-text-secondary">{t('24-hour response (Operator)')}</p>
            </div>
            <div className="space-y-0.5">
              <p className="text-xs font-semibold text-text-primary">💬 {t('Live Chat')}</p>
              <p className="text-xs text-text-secondary">{t('Mon–Fri, 9am–6pm UTC')}</p>
            ...
            <div className="space-y-0.5">
              <p className="text-xs font-semibold text-text-primary">⏱ {t('Emergency Support')}</p>
              <p className="text-xs text-text-secondary">{t('Institution plan only — 24/7 SLA')}</p>
            </div>
          </div>
