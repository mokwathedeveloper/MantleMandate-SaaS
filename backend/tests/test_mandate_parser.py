...
class TestParseMandateMocked:
    """Tests that mock the OpenRouter HTTP call — no real API key needed."""

    @patch('ai.mandate_parser._openrouter_chat')
    def test_parses_simple_mandate(self, mock_chat):
        mock_chat.return_value = '{"asset":"ETH","trigger":"RSI_below_30","risk_per_trade":0.03,"direction":"buy"}'
